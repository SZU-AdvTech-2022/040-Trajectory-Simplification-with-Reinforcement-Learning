...
class TrajComp():
    ...
    def _load(self, path, amount):
        ...
        for num in range(amount):
            self.ori_traj_set.append(to_traj(path + str(num)))
    '''
    这个函数的作用是对轨迹数据进行处理，并将处理后的结果保存在实例变量中。
    '''
    def read(self, p, episode, rem, flag):
        ...
        if flag:
            self.F_ward[m][0] = sed([self.ori_traj_set[episode][s], self.ori_traj_set[episode][rem], self.ori_traj_set[episode][m], self.ori_traj_set[episode][e]])
            self.B_ward[m][0] = sed([self.ori_traj_set[episode][s], self.ori_traj_set[episode][rem], self.ori_traj_set[episode][m], self.ori_traj_set[episode][e]])
        else:
            self.F_ward[m][0] = sed([self.ori_traj_set[episode][s], self.ori_traj_set[episode][m], self.ori_traj_set[episode][e]])
            self.B_ward[m][0] = sed([self.ori_traj_set[episode][s], self.ori_traj_set[episode][m], self.ori_traj_set[episode][e]])
        #heapq.heappush(self.heap, (self.F_ward[m][0], m))# save (state_value, point index of ori traj)
        self.sortedlist.add((self.F_ward[m][0], m))
        self.link_tail = p
    '''
    这个函数的作用是重置环境，并返回初始状态。
    '''
    ...
    def reward_update(self, episode, rem):
        if (rem not in self.start) and (rem not in self.end):
            ...
            self.start[a] = b
            self.end[b] = a
            NOW = sed(self.ori_traj_set[episode][a: b + 1])
            ...
            NOW = sed(self.ori_traj_set[episode][a: c + 1])
    ...
    def delete_heap(self, heap, nodeValue):
        ...
        if nodeValue == leafValue:
            ...
            _siftdown(heap, 0, i)
        '''
        实现对一个旅行轨迹的处理，包括在轨迹上删除一个节点、更新节点的信息、更新奖励值、更新状态信息以及进行边界处理。
        '''
    def step(self, episode, action, index, done, label = 'T'):        
        ...
        if LAST_P > self.link_head:
            ...
            self.F_ward[LAST_P][0] = sed([s, m1, m2, e])
            self.B_ward[LAST_P][0] = sed([s, m1, m2, e])
        ...
        if NEXT_P < self.link_tail:
            ...
            self.F_ward[NEXT_P][0] = sed([s, m1, m2, e])
            self.B_ward[NEXT_P][0] = sed([s, m1, m2, e])
        ...
        #_,  self.current = sed_error(self.ori_traj_set[episode], self.copy_traj) #for testing the correctness of inc rewards
    ...
    '''
    在所有三种模式中，代码都会计算出处理后的轨迹的起点和终点，然后按顺序加入处理后的轨迹中。最后，代码会调用 sed_max_e 函数来计算最终的错误值。
    '''
    def output(self, episode, label = 'T'):
        if label == 'V-VIS':
            ...
            sim_traj.append(self.ori_traj_set[episode][start])
            _, final_error = sed_max_e(self.ori_traj_set[episode], sim_traj)
            print('Validation at episode {} with error {}'.format(episode, final_error))
            #for visualization, 'sed' is by default, if you want to draw other errors by revising the codes in data_loader.py correspondingly.
            draw(self.ori_traj_set[episode], sim_traj)
            return final_error
        if label == 'V':
            start = 0
            sim_traj = []
